...
**야근 모드** — 결재 두 곳을 받고 엘리베이터를 타며 올라갑니다. 한 층 오르면 승진하고 기본 추격자가 한 명 늘어납니다. 층마다 결재 위치와 통로가 달라집니다. 잡힐 때까지 도전하며 도달 층·직급·점수를 기록합니다.
...
야근 모드에서 Q는 승진이 아니라 결재·엘리베이터 대기 시간을 버는 기술입니다. 회의가 끝나면 해당 층의 팀장님이 1.15배 빨라집니다. 승진은 엘리베이터 탑승 후 다음 층에 도착해야 이루어집니다.

Q 사용은 선택입니다. 커피와 이동만으로도 올라갈 수 있습니다. 결재를 마치고 엘리베이터에 탑승하면 안전하며, 출발까지 4초 동안 투명문 너머로 추격자를 볼 수 있습니다.

## 야근 모드 — 승진할수록 위험해집니다

![직급과 점수가 표시된 야근 모드 화면](docs/screenshots/endless.png)

1. “이쪽으로 결재받으러 오세요!” 말풍선이 있는 결재 담당자 두 명을 찾아갑니다. 서류와 도장이 놓인 창구 앞 1.8m 이내에서 **E**를 누르면 바로 결재됩니다. 담당자는 추격하지 않으며, 완료하면 말풍선이 바뀌고 서류에 도장이 표시됩니다. HUD가 아직 받지 않은 담당자의 방향·거리와 제출 안내를 보여 줍니다.
2. 복도 끝 엘리베이터 앞에서 E로 탑승합니다. 투명문이 닫히고 내부에서만 움직일 수 있으며, 밖의 추격자에게 잡히지 않습니다.
3. 4초 후 자동 출발합니다. 짧은 상승 전환 후 다음 층으로 이동하고 승진 보너스를 받습니다. E를 다시 누를 필요는 없습니다.

공통 사무실을 바탕으로 영업지원의 교차 통로, 자료관리의 긴 서가 통로, 임원지원의 지그재그 통로가 순환합니다. 결재대 위치·장애물·안내 색이 함께 바뀌며 충돌과 경로 탐색도 새로 구성됩니다. 층마다 완전히 별개의 건물을 만드는 방식은 아닙니다.

야근 추격 속도는 3.9에서 시작하며 입장 20초 뒤부터 계속 증가합니다. 65초가 지나면 해당 층의 생존 점수 적립이 멈추고 지원 인력이 도착합니다. 이후 15초마다 최대 네 차례 증원됩니다. 다음 층은 기본 추격자 2명+승진 횟수로 시작하고, 층에 따른 속도 보정은 유지하되 이전 층의 대기 시간·Q 분노는 초기화합니다. 제한시간 모드도 성인 기본 속도를 4.4, 아동을 4.8로 높였습니다.

야근 모드에서는 실내 천장등과 업무 공간은 밝게 유지되지만 창밖은 별과 초승달, 어두운 건물과 켜진 창문이 보이는 밤으로 전환됩니다. 다른 모드로 돌아가면 낮 환경으로 복구됩니다.

ESC와 창 포커스 해제는 층 체류·엘리베이터 도착·상승 시간도 함께 멈춥니다.
...
```bash
node --test tests/movement.test.cjs tests/navigation.test.cjs tests/traversal.test.cjs tests/coffee.test.cjs tests/summon.test.cjs tests/rank.test.cjs tests/floors.test.cjs
```
